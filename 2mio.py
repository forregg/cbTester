...
class firstMio(Strategy):
    ...
    def onBar(self, bar):
        ...
        e = self.engine.getHistoryBars('NZD/USD', interval, 0)
        f = self.engine.getHistoryBars('USD/CHF', interval, 0)
        ...
        en = (c - np.mean(e))/np.std(e)
        fn = (d - np.mean(f))/np.std(f)

        #print an


        r = np.matrix((an,bn,cn, dn,en,fn))
        cov = np.cov(r)
        ...
        fu = coef[0]*a + coef[1]*b + coef[2]*c + coef[3]*d + coef[4]*e+ coef[5]*f
        ...
        if f[len(fu)-1] > m + 2*s:
            ...
            if coef[4] > 0:
                self.engine.sendOrder(Order('NZD/USD', -1, 0, 0, 0,  np.round(coef[3],2), 0, 0, market=True), bar)
            else:
                self.engine.sendOrder(Order('NZD/USD', 1, 0, 0, 0,  -1*np.round(coef[3],2), 0, 0, market=True), bar)
            if coef[5] > 0:
                self.engine.sendOrder(Order('USD/CHF', -1, 0, 0, 0,  np.round(coef[3],2), 0, 0, market=True), bar)
            else:
                self.engine.sendOrder(Order('USD/CHF', 1, 0, 0, 0,  -1*np.round(coef[3],2), 0, 0, market=True), bar)
...
engine.connect(['USD/CAD', 'EUR/USD', 'GBP/USD', 'AUD/USD', 'NZD/USD', 'USD/CHF'], '1 Min', firstMio)